...
import EditNoteModal from "./EditNoteModal"; // Import the new modal component
...
export default function Sidebar() {
  ...
  const handleSelectNote = (index) => {
    ...
    if (isSelected) {
      ...
      setSelectedNotes((prev) => [...prev, index]);
    }
  };
  ...
  const handleDeleteAllSelected = () => {
    ...
    const updatedNotes = savedNotes.filter((_, index) => !selectedNotes.includes(index));
    setSavedNotes(updatedNotes);

    // Update local storage after deleting the notes
    localStorage.setItem("savedNotes", JSON.stringify(updatedNotes));

    // Clear the selected notes state
    setSelectedNotes([]);
  ...
  const handleEditTitle = (newTitle) => {
    const updatedNotes = [...savedNotes];
    updatedNotes[editIndex].title = newTitle;
    setSavedNotes(updatedNotes);
  };

  const handleEditBody = (newBody) => {
    const updatedNotes = [...savedNotes];
    updatedNotes[editIndex].body = newBody;
    setSavedNotes(updatedNotes);
  };
  ...
  return (
    ...
    <div className="relative min-h-screen lg:flex">
      {/* the edit component for editn the notes */}
      {editMode && (
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-4 rounded-md shadow-md z-10 w-11/12 max-w-md">
          <h2 className="text-2xl font-semibold mb-4 text-blue-600">Edit Note</h2>
          <input
            type="text"
            value={savedNotes[editIndex].title}
            onChange={(e) => handleEditTitle(e.target.value)}
            className="w-full mb-4 p-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
            placeholder="Enter title..."
          />
          <textarea
            value={savedNotes[editIndex].body}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end space-y-2 sm:space-y-0 sm:space-x-2">
            <button
              ...
              className="text-white bg-blue-500 px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:ring focus:border-blue-700"
            >
              Save Changes
            </button>
            <button
              onClick={handleCancelEdit}
              className="text-gray-700 px-4 py-2 rounded-md hover:bg-gray-100 focus:outline-none focus:ring focus:border-blue-300"
            >
              Cancel
            </button>
          </div>
                  ...
                  >
                    ...
                    <span className="text-lg font-semibold">
                      <button
                        ...
                        className="text-gray bg-red-100 px-4 py-2 rounded-md hover:bg-red-300 focus:outline-none focus:ring focus:border-red-700"
                      >
                        Delete All Selected
                      </button></span>
                  </a>
